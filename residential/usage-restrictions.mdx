...
A request that’s blocked due to protocol or content restrictions will fail with a `452` Disallowed Content error; a request to a blocked port fails with `453` Port Forbidden. See [Blocked Destination Categories](https://www.joinmassive.com/blocked-destinations) for the specific categories blocked at the network layer.
