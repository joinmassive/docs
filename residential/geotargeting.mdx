...
- `city` is matched by name only. If the same city name exists in more than one subdivision of the country, the request can be served from any of them. To target a specific one, `subdivision` must be specified together with `city`.
...
If the city name is not unique within the country, add `subdivision`:

```bash
curl -x https://network.joinmassive.com:65535 \
     -U '{PROXY_USERNAME}-country-US-subdivision-NY-city-New%20York:{API_KEY}' \
     https://cloudflare.com/cdn-cgi/trace
```
