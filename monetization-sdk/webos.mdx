...
| | |
| --- | --- |
| webOS SDK | [Download](https://partners.joinmassive.com/sdk/downloads) |
| webOS Sample Application | [Download](https://partners.joinmassive.com/sdk/downloads) |
...
```javascript
window.MassiveClient.isStarted().then((isStarted) => {
  // Handle started/non-started status
}).catch((error) => {
  console.error(error);
});
```
