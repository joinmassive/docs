...
| `device`      | ⬜       | **[Google only]** The name as returned by the [devices endpoint](#device-emulation) of the device to emulate searching on (these names are case insensitive but must include form- or URL-encoded spaces and punctuation marks); searching is performed on a desktop browser by default    |
...
device-specific results, rather than the default desktop results. For a list of supported
smartphones and tablets, make a request with your API token and no parameters to
