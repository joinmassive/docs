...
| Key           | Required | Value                                                                                                                                                                                                                                                                         |
| :------------ | :------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `url`         | ✅       | The URL of the page to request; any special characters require [URL encoding](https://developer.mozilla.org/en-US/docs/Glossary/Percent-encoding)                                                                                                                             |
| `device`      | ⬜       | The name as returned by the [devices endpoint](#device-emulation) of the device to emulate requesting from (these names are case insensitive but required to include form- or URL-encoded spaces and punctuation marks); device emulation is unused by default                |
...
| `mode`        | ⬜       | The synchronous or asynchronous timing of when to retrieve a response, `sync` or `async` (see [details below](#asynchronous-mode)); `sync` is the default mode                                                                                                                |
| `expiration`  | ⬜       | The maximum age of a cached response in days to accept, where `0` will disable caching; `1` is the default number of days                                                                                                                                                     |
