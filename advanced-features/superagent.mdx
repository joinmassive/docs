...
The API will provide a JSON job identifier in return:
...
If ready, the response will be returned in the
[same format as a synchronous response](#response-specification); otherwise, a JSON status message
will be returned.

A `retrieving` status indicates that the page request is in progress:

```json
{ "status": "retrieving" }
```

A `failed` status indicates that the page request was unsuccessful and the API request should be
retried:
