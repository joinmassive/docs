...
| `speed`       | ⬜       | The speed mode to browse at, `light`, `ridiculous`, or `ludicrous`; `light` is the default speed                                                                                                                                                    |
...
| `ip`          | ⬜       | The type of IP address to browse from, `residential`, `isp`, or `datacenter` (coming soon); a random IP type is used by default                                                                                                                     |
| `captcha`     | ⬜       | The CAPTCHA-handling strategy, `solved`, `ignored`, or `rejected`; `solved` is the default strategy                                                                                                                                                  |
| `readiness`   | ⬜       | The page-load event to wait for before snapshotting, `load` or `domcontentloaded`; `load` is the default readiness                                                                                                                                   |
...
Pages are returned as rendered HTML, raw (unrendered) HTML, or Markdown.
